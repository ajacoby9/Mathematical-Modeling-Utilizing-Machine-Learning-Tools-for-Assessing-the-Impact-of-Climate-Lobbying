...
def system(y, t, params):
    ...
    I_F = 345873.085
    I_A = 283377.397
    tau_F = 0.000000001112433819
    tau_A = 0.000000001112433819
...
df = pd.DataFrame(results, columns=['Beta_F', 'Beta_A', 'category'])
...
X = scaler.fit_transform(df[['Beta_F', 'Beta_A']])
y = df['category']
...
for category, color in zip(df['category'].unique(), ['#77AC30', '#A2142F', '#FFA500']):
    ...
    xx, yy = np.meshgrid(np.linspace(df['Beta_F'].min(), df['Beta_F'].max(), 100),
